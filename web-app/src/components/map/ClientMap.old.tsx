...
interface MapViewProps {
  ...
  networkData?: any
  loading?: boolean
  onElementUpdate?: (id: string, updates: any) => Promise<boolean>
}

export function ClientMap({ site, networkData: propNetworkData, loading: propLoading, onElementUpdate }: MapViewProps) {
  ...
  // Element counts for layer controls
  ...
  // Track all element names for uniqueness validation
    ...
    if (propNetworkData.poles && (selectedLayer === 'all' || selectedLayer === 'poles')) {
      propNetworkData.poles.forEach((pole: any) => {
        ...
        if (pole.pole_type === 'CUSTOMER_CONNECTION' || pole.is_connection) {
          ...
          // Desktop spec: MV poles get black borders, LV poles no border
          // White poles (st_code_1 === 0) also need black outline for visibility
          let strokeColor = '#000000' // Default black for visibility
          let weight = 1 // Default thin border
          
          // Check if this is an MV pole
          const isMV = pole.pole_type === 'MV' || pole.subnetwork?.includes('MV')
          
          // Apply borders: MV poles always get black border, LV only if white
          if (isMV) {
            strokeColor = '#000000' // Black border for MV poles
            weight = 2 // Thicker border for MV poles
          } else if (stCode1 !== 0) {
            // LV poles with non-white color don't need border
            strokeColor = color // Use same as fill (effectively no visible border)
            weight = 0 // No border for LV poles except white ones
          }
          ...
          let radius = 6 // Default LV size
          if (isMV) {
            radius = 10 // MV poles are larger
          }
